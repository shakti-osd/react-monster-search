...
function App() {
  ...
  const monsterFilter = monsters.filter(item => item?.name?.toLowerCase().includes(search.toLowerCase()))
  const monsterSorted =  monsterFilter.sort((a, b) => {
  ...
  console.log('monsterSorted',monsterSorted, order)

  return (
    <>
      <Header />
        <Container sx={{ padding: { xs: '0 16px' } }}>
          <Box sx={{ bgcolor: {xl:'#6b6464', xs: '#c6c6c6' }, height: 'auto', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }} >
